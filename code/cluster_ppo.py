...
Based on Sambhav's 10-qubit PPO code with CLI args and auto-scaling per n.

Run examples:
    python cluster_ppo.py --n 10 --episodes 100000 --seed 42 --run_name cluster10_ideal_s42
    python cluster_ppo.py --n 10 --episodes 100000 --seed 42 --noisy --run_name cluster10_noisy_s42
    python cluster_ppo.py --n 6  --episodes 50000  --seed 42 --run_name cluster6_ideal_s42
    python cluster_ppo.py --n 3  --episodes 10000  --seed 42 --run_name cluster3_ideal_s42
"""
...
def default_hparams(n):
    ...
    if n <= 3:
        ...
        return {
            ...
            'update_timestep': 10240,
            'actor_hidden': [1536, 768],
            'critic_hidden': [1536, 768],
            'batch_size': 256,
            'policy_epochs': 12,
            'actor_lr': 2e-4, 'critic_lr': 8e-4,
            'entropy_coef': 0.015,
            'gamma': 0.995,
        }
    else:  # n >= 10
        return {
            'steps_per_episode': 150,        # H decomposes to ~3 gates each → ~30 gates for init + 9 CZ + buffer = ~50 minimum, give 3x headroom
            'update_timestep': 8192,         # frequent updates
            'actor_hidden': [1024, 512],
            'critic_hidden': [1024, 512],
            'batch_size': 256,
            'policy_epochs': 15,
            'actor_lr': 3e-4, 'critic_lr': 1e-3,
            'entropy_coef': 0.05,            # high exploration for 49-action space
            'gamma': 0.99,
        }
...
def parse_args():
    ...
    p.add_argument('--entropy_decay_episodes', type=int, default=None,
                   help='Linearly decay entropy_coef from initial value to 0 '
                        'over this many episodes. Default: half of total episodes.')
    p.add_argument('--final_entropy_coef', type=float, default=0.0,
                   help='Final entropy coef after decay (default 0)')
...
# Tensor-contraction speed optimization: store state as ndim=n tensor of shape
# (2,2,...,2). Apply 2x2 single-qubit gates and 4x4 CZ-style 2-qubit gates via
# tensordot+moveaxis instead of building full 2^n x 2^n matrices each step.
# ============================================================================
class QuantumEnv(gym.Env):
    def __init__(self, num_qubits=10, noisy=False, noise_std=0.25):
        ...
        # Build action list: each is dict with 'type', 'op', 'targets', 'name', 'error_rate'
        self.actions_list = []  # list of dicts indexed by action id
        self.gate_names = []    # parallel list of names
        ...
        for i in range(self.num_qubits - 1):
            ...
            self.actions_list.append({
                ...
                'name': name, 'error_rate': 0.2,
            })
            self.gate_names.append(name)

        # Compatibility: keep `self.actions` as the dict[id->name] used elsewhere
    ...
    # ----- Tensor-contraction gate application -----
    def _apply_single_gate(self, op2, qubit):
        """In-place: apply 2x2 gate `op2` to one qubit of self.state (tensor form)."""
    ...
    def _apply_two_gate(self, op4, q1, q2):
        """Apply 4x4 gate (reshaped as (2,2,2,2)) to two qubits."""
    ...
    def step(self, action):
        ...
        # Apply gate-specific noise (Pauli rotation) if enabled
...
class PPOAgent:
    def __init__(self, n_actions, n_features, actor_lr, critic_lr,
        ...
        # entropy_coef as tf.Variable so we can update it during training
        self.entropy_coef = tf.Variable(float(entropy_coef), dtype=tf.float32,
                                        trainable=False, name='entropy_coef')
...
def save_top_sequences(all_seqs, all_fids, steps_per_ep, out_dir, topk=15):
    # Successful = found goal before max steps
...
def main():
    ...
    print(f"[ENV] n={args.n} | hilbert_dim={env.hilbert_dim} | "
    ...
    initial_entropy_coef = float(hp['entropy_coef'])
    final_entropy_coef = float(args.final_entropy_coef)
    if args.entropy_decay_episodes is not None:
        decay_episodes = int(args.entropy_decay_episodes)
    else:
        decay_episodes = args.episodes // 2  # default: decay over first half
    print(f"[ENTROPY DECAY] linear: {initial_entropy_coef} -> {final_entropy_coef} over {decay_episodes} episodes")

    print(f"--- Starting PPO Cluster({args.n} qubits, {noise_tag}) ---")

    for episode in range(args.episodes):
        # Update entropy coef BEFORE episode (linear decay)
        if decay_episodes > 0 and episode < decay_episodes:
            frac = episode / decay_episodes
            new_ent = initial_entropy_coef + frac * (final_entropy_coef - initial_entropy_coef)
        else:
            new_ent = final_entropy_coef
        agent.entropy_coef.assign(new_ent)
        obs, _ = env.reset()
        episode_actions = []
        episode_reward = 0.0
        step = 0
        ...
        if (episode + 1) % 100 == 0:
            ...
            print(f'Ep {episode+1}/{args.episodes} | Avg F {avg_f:.4f} | '
                  f'Avg Len {avg_l:.1f} | Avg R {avg_r:.2f} | '
                  f'ent={float(agent.entropy_coef.numpy()):.4f} | {elapsed:.0f}s')
