...
Includes asymmetrical, severe error rates from threeenv.py
...
class QuantumEnv(gym.Env):
    def __init__(self, num_qubits=10, noisy=False, noise_std=0.25):
        ...
        # Exact error rates provided by the user for qubits 0, 1, 2
        # For any qubit >= 3, we fall back to a default (0.05 for single, 0.2 for CZ)
        error_rates_lookup = {
            'X_0': 0.05, 'X_1': 0.28, 'X_2': 0.12,
            'SX_0': 0.07, 'SX_1': 0.12, 'SX_2': 0.29,
            'Rx(pi/2)_0': 0.03, 'Rx(pi/2)_1': 0.20, 'Rx(pi/2)_2': 0.25,
            'Rz(pi/2)_0': 0.0, 'Rz(pi/2)_1': 0.0, 'Rz(pi/2)_2': 0.0,
        ...
        for op_name, op_mat in single_ops.items():
            for i in range(self.num_qubits):
                name = f'{op_name}_{i}'
                # Use specific error rate if defined, otherwise default
                err_rate = error_rates_lookup.get(name, 0.05)
        ...
        for i in range(self.num_qubits - 1):
            name = f'CZ_{i}_{i+1}'
            err_rate = error_rates_lookup.get(name, 0.2)
            self.actions_list.append({
