...
OUTPUT_DIR = f"results_ame46_tuned_{TIMESTAMP}"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# ==============================================================================
# 1. Universal Quantum Environment (AME 4,6)
# ==============================================================================
class UniversalQuantumEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Reward shaping: stable, linear (like Code 1)
        reward = (entropy / max_possible_entropy) * 10.0

        done = entropy >= (max_possible_entropy * 0.995)
        if done:
            reward += 500.0
    ...
    def _create_universal_gate_set(self):
        ...
        omega_gold = np.exp(1j * np.pi / 10.0)
        ...
        for i in range(self.d):
            ...
            Z_mat[i, i] = omega_d ** i
            P_mat[i, i] = omega_gold ** i
            for k in range(self.d):
                F_mat[i, k] = omega_d ** (i * k)
        F_mat /= np.sqrt(self.d)

        primitives = {'X': X_mat, 'Z': Z_mat, 'F': F_mat, 'P': P_mat}
...
class PPOAgent:
    def __init__(
        ...
        initial_lr=1e-4,
        final_lr=1e-6,
        decay_steps=100000,   # polynomial decay steps (keep simple: tied to EPISODES)
        gamma=0.99,
        clip_ratio=0.1,
        policy_epochs=10,
        batch_size=4096,
        ...
        # Polynomial LR decay (as requested)
        ...
        # Entropy coef (changed from 0.08 -> 0.02)
        self.initial_entropy = initial_entropy
        self.update_counter = 0  # increments per PPO update
    ...
    def learn(self):
        ...
        # Entropy decay (yes, entropy changes over training)
        self.update_counter += 1
        entropy_coef = self.initial_entropy * (0.9995 ** self.update_counter)
...
# ==============================================================================
# 3. Plot Saving (training curve + histogram like Code 1)
# ==============================================================================
def save_plots(all_entropies, successful_sequences, env):
    k = env.n // 2
    max_theoretical = k * np.log(env.d)

    # Training curve + moving average + limit line
    ...
    plt.axhline(y=max_theoretical, linestyle='--', label=f'AME Limit ({max_theoretical:.4f})')
    ...
    # Histogram of top successful sequences
...
if __name__ == '__main__':
    ...
    agent = PPOAgent(
        ...
        initial_lr=1e-4,
        final_lr=1e-6,
        decay_steps=EPISODES,     # polynomial decay over episodes (simple + stable)
        gamma=0.99,
        clip_ratio=0.1,
        policy_epochs=10,
        batch_size=4096,
    ...
    print(f"--- Starting AME({NUM_QUDITS}, {DIM}) Search on Bulbasaur ---")
    ...
    for episode in range(EPISODES):
        ...
        for step in range(STEPS_PER_EPISODE):
            ...
            if timestep_counter % UPDATE_TIMESTEP == 0:
                print(f"[Update] Training at step {timestep_counter} ...")
                agent.learn()
        ...
        # Save "successful" sequences relative to the theoretical max (like Code 1)
