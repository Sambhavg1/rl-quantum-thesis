...
OUTPUT_DIR = f"results_ame_pudda_trimmed_{TIMESTAMP}"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# ==============================================================================
# 1. QUANTUM ENVIRONMENT (PUDDA TRIMMED ACTION SPACE)
# ==============================================================================
class UniversalQuantumEnv(gym.Env):
    ...
    def _create_tensor_gate_set(self):
        ...
        # --- TRIMMED ACTION SPACE: Only ONE irrational angle ---
        theta = np.pi / 5.0
        ...
        for j in range(self.d):
            ...
            P_mat[j, j] = np.exp(1j * j * theta)
            for k in range(self.d):
                H_mat[j, k] = omega_d ** (j * k)
                
        H_mat /= np.sqrt(self.d)

        # 1. Append all Single Gates
        single_primitives = {'X': X_mat, 'Z': Z_mat, 'H': H_mat, 'S': S_mat, 'T': T_mat, 'K': K_mat, 'P_pi5': P_mat}
        for name, mat in single_primitives.items():
            for i in range(self.n):
                gates.append({'type': 'single', 'targets': (i,), 'tensor': mat})
                names.append(f"{name}_{i}")
                
        # 2. Setup Two-Qudit Tensors
        CX_tensor = np.zeros((self.d, self.d, self.d, self.d), dtype=np.complex128)
        CZ_tensor = np.zeros((self.d, self.d, self.d, self.d), dtype=np.complex128)
        SWAP_tensor = np.zeros((self.d, self.d, self.d, self.d), dtype=np.complex128)
        SUMP_tensor = np.zeros((self.d, self.d, self.d, self.d), dtype=np.complex128)

        for c in range(self.d):
            for t in range(self.d):
                CX_tensor[c, (t + c) % self.d, c, t] = 1.0
                CZ_tensor[c, t, c, t] = omega_d ** (c * t)
                SWAP_tensor[t, c, c, t] = 1.0
                SUMP_tensor[c, t, c, t] = np.exp(1j * c * t * theta)

        pairs = list(combinations(range(self.n), 2))
        for i, j in pairs:
            # CX is directional (Controlled-Shift)
            gates.append({'type': 'two', 'targets': (i, j), 'tensor': CX_tensor})
            names.append(f"CX_{i}_{j}")
            gates.append({'type': 'two', 'targets': (j, i), 'tensor': CX_tensor})
            names.append(f"CX_{j}_{i}")
            
            # CZ, SWAP, and SUMP are symmetric 
            gates.append({'type': 'two', 'targets': (i, j), 'tensor': CZ_tensor})
            names.append(f"CZ_{i}_{j}")
            
            gates.append({'type': 'two', 'targets': (i, j), 'tensor': SWAP_tensor})
            names.append(f"SWAP_{i}_{j}")
            
            gates.append({'type': 'two', 'targets': (i, j), 'tensor': SUMP_tensor})
            names.append(f"SUMP_pi5_{i}_{j}")
...
class PPOAgent:
    def __init__(self, n_actions, n_features, initial_lr=3e-4, steps_per_cycle=10000,
                 gamma=0.99, clip_ratio=0.1, policy_epochs=10, batch_size=4096, initial_entropy=0.10):
    ...
    def _build_actor(self):
        ...
        # Reduced from 3 layers back to 2, since action space is 58, not 142
...
def save_plots(all_entropies, successful_sequences, env):
    ...
    plt.title(f'Training Progress: Trimmed Pudda Set (58 Actions) (N={env.n}, D={env.d})')
    plt.xlabel('Episode')
...
if __name__ == '__main__':
    ...
    # 120 steps is plenty for 58 actions
    STEPS_PER_EPISODE = 120
    # Update more frequently since action space is smaller
    ...
    agent = PPOAgent(
        ...
        clip_ratio=0.1, policy_epochs=10, batch_size=4096, initial_entropy=0.10       
    )

    print(f"--- Starting AME({NUM_QUDITS}, {DIM}) Search ---")
    print(f"Gate Set: TRIMMED Pudda et al. 2024. Total Actions: {env.action_space.n}. Horizon: {STEPS_PER_EPISODE}.")
