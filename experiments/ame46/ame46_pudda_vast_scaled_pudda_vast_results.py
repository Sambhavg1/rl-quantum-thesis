...
class UniversalQuantumEnv(gym.Env):
    ...
    def _create_tensor_gate_set(self):
        ...
        # 10 Distinct Angles for P(theta) and SUMP(theta)
        angles = {
            "pi_10": np.pi / 10.0,
            "pi_8":  np.pi / 8.0,
            "pi_7":  np.pi / 7.0,
...
# ==============================================================================
# 2. PPO AGENT (SCALED FOR 142 ACTIONS)
# ==============================================================================
class PPOAgent:
    def __init__(self, n_actions, n_features, initial_lr=3e-4, steps_per_cycle=10000,
                 gamma=0.99, clip_ratio=0.1, policy_epochs=10, batch_size=4096, initial_entropy=0.15):
    ...
    def _build_actor(self):
        ...
        # <--- ADDED LAYER: Deeper network to handle 142 actions gracefully --->
    ...
    def _build_critic(self):
        ...
        x = layers.Dense(2048, activation='relu')(inputs)
        x = layers.Dense(1024, activation='relu')(x)
        # <--- ADDED LAYER: Matching depth for the critic --->
        x = layers.Dense(512, activation='relu')(x)
...
if __name__ == '__main__':
    ...
    # <--- INCREASED HORIZON & REPLAY BUFFER --->
    STEPS_PER_EPISODE = 200 # Increased from 120 to allow longer sequence assembly
    UPDATE_TIMESTEP = 16384 # Increased from 8192 to collect more data per policy update
    ...
    agent = PPOAgent(
        ...
        clip_ratio=0.1, policy_epochs=10, batch_size=4096, 
        initial_entropy=0.15 # <--- BUMPED ENTROPY: Encourages exploring all 142 actions 
    )
