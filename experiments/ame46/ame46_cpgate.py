...
OUTPUT_DIR = f"results_ame_ultimate_{TIMESTAMP}"
...
class UniversalQuantumEnv(gym.Env):
    def __init__(self, num_qudits=4, dim=6):
        ...
        # Precompute partial trace permutations for extreme speed
    ...
    def reset(self):
        # State is an N-dimensional tensor: shape (6, 6, 6, 6)
    ...
    def step(self, action):
        ...
        # Fast Tensor Contraction applies gates without full matrix multiplication
        if gate_type == 'single':
    ...
    def _create_tensor_gate_set(self):
        ...
        # Standard Clifford SUM Gate (D x D x D x D Tensor)
        ...
        # Non-Clifford Entangling Gate (Fractional CP)
        CP_gold_tensor = np.zeros((self.d, self.d, self.d, self.d), dtype=np.complex128)
        for c in range(self.d):
            for t in range(self.d):
                phase = np.exp(1j * np.pi * c * t / 5.0) # Irrational entanglement
                CP_gold_tensor[c, t, c, t] = phase

        pairs = list(combinations(range(self.n), 2))
        for i, j in pairs:
            gates.append({'type': 'two', 'targets': (i, j), 'tensor': SUM_tensor})
            names.append(f"SUM_{i}_{j}")
            gates.append({'type': 'two', 'targets': (i, j), 'tensor': CP_gold_tensor})
            names.append(f"CPgold_{i}_{j}")
...
if __name__ == '__main__':
    ...
    print(f"--- Starting Ultimate AME({NUM_QUDITS}, {DIM}) Search ---")
    print(f"Horizon: {STEPS_PER_EPISODE} steps. Tensors Enabled. CPgold Entangler Active.")
    ...
    for episode in range(EPISODES):
        ...
        # <--- REVERTED HERE: Back to strict 99% threshold for clean CSV output --->
    ...
    # Sort descending by entropy so best result is at the top of the CSV
