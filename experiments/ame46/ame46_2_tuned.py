...
class UniversalQuantumEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # <--- NON-HYPERPARAMETER FIX: POWER-LAW REWARD SHAPING --->
        # Punishes the 3.2 plateau harshly. 3.2 -> ~33 points. 3.58 -> 100 points.
    ...
    def _create_universal_gate_set(self):
        ...
        # <--- NON-HYPERPARAMETER FIX: EXPANDED NON-CLIFFORD SET --->
        omega_gold_1 = np.exp(1j * np.pi / 10.0) 
        omega_gold_2 = np.exp(1j * np.pi / 5.0)   # Larger step
        omega_gold_3 = np.exp(1j * np.pi / 7.0)   # Grid breaker
...
class PPOAgent:
    def __init__(
        ...
        n_actions,
        n_features,
        initial_lr=3e-4,       # <--- HYPERPARAMETER: Slightly higher start
        steps_per_cycle=10000, # <--- HYPERPARAMETER: For Cosine Restart
        gamma=0.99,
        clip_ratio=0.1,
        policy_epochs=10,
        batch_size=4096,
        initial_entropy=0.1    # <--- HYPERPARAMETER: Huge exploration boost
    ):
        ...
        # <--- HYPERPARAMETER: CYCLIC LR --->
        # Prevents getting stuck in the plateau by spiking the LR periodically
    ...
    def learn(self):
        ...
        # Cyclic entropy decay: drops, then pops back up slightly
...
if __name__ == '__main__':
    ...
    print(f"--- Starting Tuned AME({NUM_QUDITS}, {DIM}) Search on Bulbasaur ---")
