...
OUTPUT_DIR = f"results_ame62_converge_{TIMESTAMP}"
...
class UniversalQuantumEnv_Masked(gym.Env):
    def __init__(self, num_qudits=6, dim=2):
        super().__init__()
        self.n = num_qudits
        self.d = dim
...
# ==============================================================================
# 2. PPO AGENT (Robust Hyperparameter Tuning)
# ==============================================================================
class MaskedPPOAgent:
    def __init__(self, n_actions, n_features, actor_lr=1e-4, critic_lr=5e-4, decay_steps=10000):
        self.n_actions = n_actions
        self.n_features = n_features
        self.gamma = 0.99
        
        # ROBUSTNESS UPDATES
        self.clip_ratio = 0.1       # More conservative policy updates
        self.policy_epochs = 20     # Increased training on each batch
        self.batch_size = 512
        self.initial_entropy = 0.02
        
        # Split and lowered learning rate schedules
        self.actor_lr_schedule = tf.keras.optimizers.schedules.PolynomialDecay(
            initial_learning_rate=actor_lr, decay_steps=decay_steps, end_learning_rate=1e-6, power=1.0)
            
        self.critic_lr_schedule = tf.keras.optimizers.schedules.PolynomialDecay(
            initial_learning_rate=critic_lr, decay_steps=decay_steps, end_learning_rate=1e-6, power=1.0)
...
def save_results(all_entropies, successful_sequences, env):
    ...
    plt.axhline(y=max_theoretical, color='green', linestyle='--', linewidth=2, label=f'AME Limit ({max_theoretical:.4f})')
    plt.title(f'Physics-Informed Training (Converging) (N={env.n}, D={env.d})')
    plt.xlabel('Episode')
    ...
    if successful_sequences:
        csv_path = os.path.join(OUTPUT_DIR, "converged_best_sequences.csv")
...
if __name__ == '__main__':
    # HORIZON & EPISODE SCALING FOR N=6
    EPISODES = 30000            
    STEPS_PER_EPISODE = 60
    UPDATE_TIMESTEP = 16384      
    
    env = UniversalQuantumEnv_Masked(num_qudits=6, dim=2)
    agent = MaskedPPOAgent(
        n_actions=env.action_space.n, n_features=env.observation_space.shape[0],
        actor_lr=1e-4, critic_lr=5e-4, decay_steps=EPISODES
    )

    print(f"--- Starting Action-Masked AME(6,2) Convergence Run ---")
    ...
    for episode in range(EPISODES):
        ...
        if episode_max_entropy >= (env.target_metric_max * 0.999):
        ...
        if (episode + 1) % 100 == 0:
            ...
            print(f'Ep: {episode + 1}/{EPISODES} | Avg Max: {avg_ent:.4f} | Actor LR: {curr_lr:.2e} | Time: {time.time()-start_time:.1f}s')
