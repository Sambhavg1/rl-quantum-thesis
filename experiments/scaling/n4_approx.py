...
OUTPUT_DIR = f"results_approx_N4_D2_{TIMESTAMP}"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# ==============================================================================
# 1. ENVIRONMENT: Expanded Fractional Gate Library
# ==============================================================================
class QuantumEnv_Approximation(gym.Env):
    def __init__(self, num_qudits=4, dim=2):
        super().__init__()
        self.n = num_qudits
        self.d = dim
        ...
        self.gates, self.gate_names = self._create_fractional_gate_set()
    ...
    def step(self, action):
        ...
        # Pure dense reward gradient (Sparse bonus removed for impossible states)
        reward = (quality ** 10.0) * 100.0 
        reward -= 0.05 
        
        done = False # Agent must use all steps to carve out the best approximation
    ...
    def get_action_mask(self):
        ...
        if self.last_action is not None:
            ...
            # Prune exact self-inverses for standard gates
            if last_info['type'] == 'single_std':
                mask[self.last_action] = 0.0
                
            # Prevent stacking fractional rotations on the same axis
            if last_info['type'] == 'single_Rx':
                q_last = last_info['targets'][0]
                for a in range(self.action_space.n):
                    if self.gates[a]['type'] == 'single_Rx' and self.gates[a]['targets'][0] == q_last:
                        mask[a] = 0.0
                        
            if last_info['type'] == 'single_Rz':
                q_last = last_info['targets'][0]
                for a in range(self.action_space.n):
                    if self.gates[a]['type'] == 'single_Rz' and self.gates[a]['targets'][0] == q_last:
                        mask[a] = 0.0

            # Canonical Ordering 
            if 'single' in last_info['type']:
                q_last = last_info['targets'][0]
                for a in range(self.action_space.n):
                    if 'single' in self.gates[a]['type']:
    ...
    def _create_fractional_gate_set(self):
        gates, names = [], []
        
        # 1. Standard Primitives
        X_mat = np.array([[0, 1], [1, 0]], dtype=np.complex128)
        Z_mat = np.array([[1, 0], [0, -1]], dtype=np.complex128)
        F_mat = np.array([[1, 1], [1, -1]], dtype=np.complex128) / np.sqrt(2)
        
        for name, mat in {'X': X_mat, 'Z': Z_mat, 'F': F_mat}.items():
            for i in range(self.n):
                gates.append({'type': 'single_std', 'targets': (i,), 'tensor': mat})
                names.append(f"{name}_{i}")

        # 2. Fractional Rotations (Rx and Rz)
        angles = [np.pi/4, np.pi/8, -np.pi/4, -np.pi/8]
        angle_names = ["pi/4", "pi/8", "-pi/4", "-pi/8"]
        
        for theta, ang_name in zip(angles, angle_names):
            ...
            for i in range(self.n):
                gates.append({'type': 'single_Rx', 'targets': (i,), 'tensor': Rx})
                names.append(f"Rx({ang_name})_{i}")
                gates.append({'type': 'single_Rz', 'targets': (i,), 'tensor': Rz})
                names.append(f"Rz({ang_name})_{i}")

        # 3. Bidirectional SUM Gates
...
# ==============================================================================
# 2. PPO AGENT (Standard Robust Setup)
# ==============================================================================
class MaskedPPOAgent:
    def __init__(self, n_actions, n_features, decay_steps):
        ...
        self.gamma = 0.99
        self.clip_ratio = 0.1       
        self.policy_epochs = 20     
        self.batch_size = 512
        self.initial_entropy = 0.05
        
        self.actor_lr_schedule = tf.keras.optimizers.schedules.PolynomialDecay(
            initial_learning_rate=1e-4, decay_steps=decay_steps, end_learning_rate=1e-6, power=1.0)
        self.critic_lr_schedule = tf.keras.optimizers.schedules.PolynomialDecay(
            initial_learning_rate=5e-4, decay_steps=decay_steps, end_learning_rate=1e-6, power=1.0)
    ...
    def learn(self):
        ...
        self.step_counter += 1
        entropy_coef = max(0.0, self.initial_entropy * (1 - self.step_counter / (self.actor_lr_schedule.decay_steps // 2048)))
...
# ==============================================================================
def save_results(all_entropies, best_sequences, env):
    ...
    if len(all_entropies) >= 100:
        ...
        plt.plot(np.arange(99, len(all_entropies)), moving_avg, color='red', linewidth=2, label='Moving Avg (100)')
    plt.axhline(y=max_theoretical, color='green', linestyle='--', linewidth=2, label=f'Theoretical Limit ({max_theoretical:.4f})')
    plt.title(f'N=4 Approximation Run (with Fractional Rotations)')
    plt.xlabel('Episode')
    plt.ylabel('Von Neumann Entropy (Nats)')
    plt.legend()
    plt.grid(True)
    
    plot_path = os.path.join(OUTPUT_DIR, "approx_training_curve.png")
    plt.savefig(plot_path)
    plt.close()
    
    if best_sequences:
        csv_path = os.path.join(OUTPUT_DIR, "approx_best_sequences.csv")
        best_sequences.sort(key=lambda x: x[1], reverse=True)
        with open(csv_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Episode_Entropy', 'Sequence_Length', 'Gate_Sequence'])
            for seq_tuple, ent in best_sequences:
...
if __name__ == '__main__':
    ...
    env = QuantumEnv_Approximation(num_qudits=4, dim=2)
    agent = MaskedPPOAgent(
        n_actions=env.action_space.n, n_features=env.observation_space.shape[0],
        decay_steps=EPISODES
    )

    print(f"--- Starting N=4 Approximation Search (Fractional Gates Added) ---")
    ...
    for episode in range(EPISODES):
        ...
        if episode_max_entropy > global_max_entropy:
            ...
            print(f"!!! NEW APPROX RECORD: {global_max_entropy:.4f} (Episode {episode+1}) !!!")

        # Save any high-quality approximations
        if episode_max_entropy >= (env.target_metric_max * 0.70):
            best_sequences.append((tuple(episode_actions), episode_max_entropy))

        if (episode + 1) % 100 == 0:
            avg_ent = np.mean(all_entropies[-100:])
            curr_lr = agent.actor_lr_schedule(agent.actor_optimizer.iterations).numpy()
    ...
    # Keep only top 100 sequences to avoid massive files
    save_results(all_entropies, best_sequences[:100], env)
