...
# ==============================================================================
# 2. PPO Agent (Optimized for GPU Batching)
# ==============================================================================
class PPOAgent:
...
# ==============================================================================
# 3. Visualization & Analysis Functions (SERVER SAFE)
# ==============================================================================
def save_plots(all_entropies, successful_sequences, env):
    """Saves training curve and histogram to PNG files instead of showing them."""
    k = env.n // 2
    ...
    if successful_sequences:
        ...
        for bar, entropy in zip(bars, avg_entropies):
            yval = bar.get_height()
            plt.text(bar.get_x() + bar.get_width()/2, yval + 0.5, f"S={entropy:.2f}", ha='center', va='bottom', fontsize=9, color='darkred')
...
if __name__ == '__main__':
    ...
    # Increased UPDATE_TIMESTEP to utilize GPU memory better
    ...
    for episode in range(EPISODES):
        ...
        for step in range(STEPS_PER_EPISODE):
            ...
            if timestep_counter % UPDATE_TIMESTEP == 0:
                print(f"[GPU Update] Training on collected experience at Step {timestep_counter}...")
                agent.learn()
        ...
        if (episode + 1) % 100 == 0:
            ...
            print(f'Episode: {episode + 1}/{EPISODES} | Avg Max Ent: {avg_ent:.4f} | LR: {curr_lr:.2e} | Time: {elapsed:.1f}s')
